...
/*  function testFN() {
              ...
              console.log('Captured ' + stuff.length + 'bits!');
              //stuff += data;
              var loopI = i * 1024;
            ...
            console.log(stuff);
            i = 0;
            return _this.bytesRead += stuff.length;
    ...
    function RTMPProtocol(stream) {
      ...
      this.socket.on('data', function(data) {
        if (data.length == 1024) {
            ...
            console.log(stuff);
            i++;
            console.log('Captured 1k bits - stream!');
          }
/*          console.log('RTMPClient if (this.socket): data + length');
          console.log(data);
          console.log(data.length);
          var testvar = _this.bytesRead + data.length;
          console.log('Testvar:')
          console.log(testvar);*/
          if (data.length !== 1024) {
    ...
    RTMPProtocol.prototype.parseMessages = function(data) {
      ...
      while (offset < data.length) {
        ...
        if (oldData) {
          ...
          console.log('if oldData - packetData, oldData, newData:');
          console.log(packetData);
          console.log(oldData);
          console.log(newData);
        } else {
          packetData = newData;
          console.log('if oldData - newData:');
          console.log(newData);
        }
...
/*          if (data.length == 1024) {
            stuff += data;
            i++;
            console.log('Captured 1k bits - socket!');
          ...
          console.log(data.length);
          var testvar = _this.bytesRead + data.length;
          var testvar3 = testvar3 + data.length;
          console.log('Testvar:')
          console.log(testvar);
...
/*          if (data.length !== 1024) {
              console.log('Captured ' + data.length + 'bits!');
              stuff += data;
              console.log('Stuff = ' + stuff.length + 'bits!');
              var loopI = i * 1024;
              var total = loopI + data.length;
            console.log('LENGTH OF DATA: ' +  total);*/
            //console.log('DATA TYPE: ' + typeof data);
    ...
    RTMPClient.prototype.handshake = function(cb) {
      ...
      handleHandshake1 = function(data) {
        if (data.length == 1024) {
            ...
            testvar2 = _this.bytesRead + data.length;
            console.log('Testvar2:')
            console.log(testvar2);
            testvar4 = testvar4 + string.length;
            console.log('Testvar4:')
            console.log(testvar4);
            if (i == 0){
              console.log('First time through newBuffer!');
              newBuffer = data;
            ...
            if (i > 0){
              x = i+1
              bcLength = x * 1024;
              console.log(x + ' time through newBuffer!');
              //var testBuffer = new Buffer(1);
              newBuffer = Buffer.concat([newBuffer, data]);
            }
            i++;
            console.log('Captured 1k bits - handshake!');
            console.log(typeof data);
            console.log(typeof stuff);
            //console.log(stuff);
            //setTimeout(testFN(), 1000)
...
/*          console.log('RTMPClient if (this.socket): data + length');
          ...
          if (data.length !== 1024) {
              ...
              console.log('Testvar2:')
              console.log(testvar2);
              var testvar4 = testvar4 + string.length;
              console.log('Testvar4:')
              console.log(testvar4);
              console.log('Captured ' + data.length + 'bits - concat!');
              //stuff += data;
              //stuff = stuff + data;
              //data[i] = data;
              //Buffer.concat[data];
              newBuffer = Buffer.concat([newBuffer, data])
              console.log('newBuffer = ' + newBuffer.length + 'bits - total!');
              var loopI = i * 1024;
              var total = loopI + data.length;
        ...
        if (newBuffer.length !== 1537 && newBuffer.length !== 3073 /*delete this -->*/ /*&& data.length !== 1024*/) {
          //console.log(data);
          cb(new Error('Handshake response was incorrect size.' + newBuffer.length));
        }
        if (newBuffer.length === 1537) {
          clientResponse = newBuffer.slice(1);
...
/*                if (data.length === 1024) {
        ...
        //////////////////////////////////////////////
        if (newBuffer.length === 3073) {
          clientResponse = newBuffer.slice(1, 1537);
          return handleHandshake2(newBuffer.slice(1537));
        }
            //return _this.bytesRead += stuff.length;
          }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////        
//         _this.socket.removeListener('data', handleHandshake1);
//         if (data.length !== 1537 && data.length !== 3073 /*delete this -->*/ /*&& data.length !== 1024*/) {
//           //console.log(data);
//           cb(new Error('Handshake response was incorrect size.' + data.length));
//         }
//         if (data.length === 1537) {
//           clientResponse = data.slice(1);
//           _this.socket.addListener('data', handleHandshake2);
...
//       };
      handleHandshake2 = function(data) {
        ...
        if (data.length !== 1536) {
          return cb(new Error('Handshake response was incorrect size - hH2:' + data.toString('base64')));
        } else {
